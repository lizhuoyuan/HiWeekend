...
public class CinemaInfoFragment extends Fragment implements OnTouchListener,
	OnPageChangeListener, OnClickListener, BookClickedListener,
	...
	juHeUtils.getMovies(getContext(), cinemaID, false);
	return view;
	...
	listAdapter = new Row_of_tablets_listAdapter(broadcasts, getContext());
	...
	if ((count = seats.size()) <= 0) {
	    Toast.makeText(getContext(), "您没有选择座位", 0).show();
	    return;
	}
	if (!PreferenceManager.getDefaultSharedPreferences(getContext())
	...
	juHeUtils.getMovies(getContext(), cinemaID, flag);
