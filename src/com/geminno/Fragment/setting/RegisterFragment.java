...
public class RegisterFragment extends Fragment implements OnClickListener,
	...
	if (TextUtils.isEmpty(u_name.getText().toString())) {
	    Toast.makeText(getContext(), "用户名不正确，请重新输入", Toast.LENGTH_SHORT)
		    .show();
	    return;
	}
	...
	if (pwdResult == 1) {
	    Toast.makeText(getContext(), "两次输入的密码不一致，请确认！", Toast.LENGTH_LONG)
		    .show();
	    return;
	}
	if (pwdResult == 2) {
	...
	if (TextUtils.isEmpty(u_tel.getText().toString())
	    ...
	    Toast.makeText(getContext(), "请输入正确的手机号码！", Toast.LENGTH_LONG)
		    .show();
	    return;
	}
	...
	if (TextUtils.isEmpty(pay_pwd.getText().toString())) {
	    Toast.makeText(getContext(), "支付密码不能为空！", Toast.LENGTH_LONG).show();
	    return;
	}
	...
	new Thread(new Runnable() {
	    String url = InternetService.ServletURL + "UserRegister";
